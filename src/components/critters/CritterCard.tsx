...
const RARITY_BORDER: Record<string, string> = {
  common: 'border-green-200',
  uncommon: 'border-blue-200',
  rare: 'border-red-200',
};
...
const WEATHER_COLORS: Record<string, string> = {
  ...
  hurricane: 'bg-indigo-100 text-indigo-700',
};
...
export function CritterCard({ critter, critterCategories }: CritterCardProps) {
  const { caughtCritters, toggleCaught } = useCaughtCritterStore();
  const isCaught = caughtCritters.includes(critter.id);
  const [isHovered, setIsHovered] = useState(false);

  const rarityName = getLabelFromCategories(critterCategories.rarities, critter.rarity);
  const typeName = getLabelFromCategories(critterCategories.types, critter.type);
  ...
  return (
    ...
    >
      ...
      <div className={`hidden tablet:flex flex-row gap-0.5 absolute -top-9 right-4 transition-all duration-200 ease-out ${
        ...
        <button
          onClick={(e) => { e.stopPropagation(); toggleCaught(critter.id); }}
      ...
      {/* Card content */}
      <div className={`relative z-10 rounded-xl border p-3 transition-shadow hover:shadow-md ${RARITY_BORDER[critter.rarity]} ${RARITY_BG[critter.rarity]}`}>
        {/* Header row */}
        <div className="relative flex items-start gap-3 mb-3">
          {/* Critter icon */}
          <div className="w-16 h-16 flex-shrink-0 relative">
            <div className={`w-full h-full rounded-xl flex items-center justify-center overflow-hidden ${RARITY_ICON_BG[critter.rarity]}`}>
              <img
                src={`/images/critters/${critter.icon}`}
                alt={critter.name}
          ...
          <div className="flex-1 min-w-0 flex flex-col gap-1">
            <h3 className="font-semibold text-sm text-[var(--color-text)] flex items-center gap-1.5">
              ...
              <span className="truncate">{critter.name}</span>
            </h3>
            <div className="text-xs font-medium text-[var(--color-text-muted)]">
              {typeName}
            </div>
            <span className={`inline-flex items-center px-1.5 py-0.5 rounded text-xs font-medium w-fit ${RARITY_COLORS[critter.rarity]}`}>
              {rarityName}
            </span>
          </div>
          {/* Price - top right */}
        ...
        {/* Divider */}
        <div className={`border-t mb-3 ${RARITY_BORDER[critter.rarity]}`}></div>
        ...
        <div className="flex flex-col gap-2">
          ...
          {critter.notes && (
            <div className="flex items-start gap-1.5">
              ...
              <span className="text-[10px] text-[var(--color-text-muted)] italic">{critter.notes}</span>
            </div>
          )}
        </div>
      </div>
