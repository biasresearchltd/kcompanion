...
interface SwipeableFishCardProps {
  fish: Fish;
...
export function SwipeableFishCard(props: SwipeableFishCardProps) {
  const { fish } = props;
  ...
  const updatePosition = useCallback((translateX: number) => {
    ...
    if (translateX < 0) {
      ...
      // Reset icon when not swiping left
      if (caughtIconRef.current) {
  ...
  const handleTouchEnd = useCallback(() => {
    ...
    touchStartYRef.current = null;
    isSwipingRef.current = false;
  }, [isAnimating, toggleCaught, fish.id, updatePosition]);
  ...
  return (
    <div className="relative overflow-hidden">
      {/* Caught icon container - positioned on the RIGHT (swipe left reveals) */}
      <div
        ref={caughtIconRef}
      ...
      >
        <div className={showCaughtFillAnimation ? 'scale-125' : ''} style={{ transition: 'none' }}>
          <div className="relative w-8 h-8">
            {/* Green fill with clip-path animation */}
            <svg
              ref={caughtFillRef}
